...
get_switch_error = function(haplotypes_output_file, haplotypes_test_file, random=T){
  ...
  for(i in seq(1, ncol(haplotypes_output), by=2)) {
    ...
    # get vector with all the heterozygous positions
    het_sites = haplotypes_test[,i] + haplotypes_test[,(i+1)] == 1

    # look only at the firs position and see if both haplotypes are aligned
    align     = haplotypes_test_hets[1,i] == haplotypes_output_hets[1,i]
    
    # select from the list of heterozygous positions
...
#/usr/local/bin/Rscript calculate_switch_accuracy.R ../data/example_data_2_100_sol_j4.txt ../data/example_data_2_100_sol.txt